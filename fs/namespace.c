...
static void mnt_free_id(struct mount *mnt)
{
	...
	if (id >= DEFAULT_KSU_MNT_ID) {
		ida_simple_remove(&susfs_mnt_id_ida, id);
		return;
	}
...
static int mnt_alloc_group_id(struct mount *mnt)
{
	...
	if (susfs_is_current_ksu_domain()) {
		res = ida_simple_get(&susfs_mnt_group_ida, DEFAULT_KSU_MNT_GROUP_ID, 0, GFP_KERNEL);
		if (res < 0)
			return res;
...
void mnt_release_group_id(struct mount *mnt)
{
	...
	if (id >= DEFAULT_KSU_MNT_GROUP_ID) {
		ida_simple_remove(&susfs_mnt_group_ida, id);
		mnt->mnt_group_id = 0;
		return;
	}
#endif // #ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
